...
### Step 4.5: 即梦提示词合规化处理（中国大陆审核）

**即梦 Seedance 2.0 运行在中国大陆服务器，受中国互联网内容监管。所有即梦提示词在输出前必须经过合规化处理，否则会被平台拦截导致生成失败。**

#### 合规化处理流程（5 步）

**第 1 步：硬拦截内容检测（不可修复，必须替换）**

以下内容会触发平台级前置拦截，与提示词写法无关：
- **可识别的真实人脸**：名人、政客、公众人物 → 改为虚构人物描述，不使用任何真实姓名
- **受版权保护的角色**：迪士尼角色、漫威/DC英雄等 → 改为通用描述"穿红蓝色紧身衣的超级英雄"
- **政治敏感内容**：任何涉及政治人物、政治事件、国旗/政治符号 → 完全避免

**第 2 步：敏感词汇替换**

| ❌ 高风险词（必换） | ✅ 安全替代 |
|---------------------|------------|
| 儿童、小孩、boy、child、loli | 年轻人、青少年、少年、人物 |
| 血、流血、blood | 红色液体、伤痕（如果必须） |
| 暴力、暴力行为、violence | 激烈动作、冲突场面 |
| 杀、谋杀、死亡、kill | 倒下、失去意识（如果必须）
...
**关键技巧：加入影视制作术语（镜头/灯光/画幅/风格）能显著提升通过率**，因为系统会将提示词识别为专业影视创作内容而非写实记录。

**第 4 步：负面提示词后缀（每个即梦提示词末尾必须附加）**

```
电影质感，高质量，物理真实，角色一致性强，无不良内容
```

**第 5 步：简短提示词扩展**

即梦的过滤机制基于语言模型整体理解场景。如果提示词过于简短（少于 30 字），系统因无法判断意图而倾向保守拦截。
- **最简扩展模板**：`[场景地点]，[人物描述]，[动作]，[光影氛围]，[镜头风格]，电影质感，高质量`
...
- [ ] 不含版权角色名（迪士尼、漫威、DC等）
- [ ] 不含政治敏感词
- [ ] 不含 child/boy/loli 等低龄词汇（用"年轻人"替代）
- [ ] 不含 blood/violence/kill 等暴力词汇（已用电影语境包裹）
- [ ] 不含裸露/性暗示相关词
- [ ] 不含赌博/毒品相关词
- [ ] 已加入影视制作术语建立创作语境
- [ ] 已附加负面提示词后缀
